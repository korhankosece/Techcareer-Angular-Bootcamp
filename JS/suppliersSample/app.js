...
const numbers = suppliers.map(supplier => {
    ...
    number = number.replace('(', '').replace(')', '').replaceAll(' ', '').replace('-', '').replaceAll('.', '')
    return +number
})

console.log(numbers);

suppliers.map(supplier => supplier.address?.country?.toLowerCase() === 'usa' ? console.log(supplier.contactName.toUpperCase()) : '')